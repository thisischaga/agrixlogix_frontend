// Actions rapides → routes réelles (transactions / votes)
import { Link } from 'react-router-dom';
import { Send, Download, ClipboardList } from 'lucide-react';
      ...
      <div className="flex flex-col gap-2.5 flex-1">
        {actions.map(({ id, to, label, sub, icon: Icon, bg, iconBg, text }) => (
          <Link
            key={id}
            to={to}
            className={`flex items-center gap-3 p-4 rounded-xl ${bg} ${text} w-full text-left no-underline transition-all duration-200 hover:brightness-[0.98] hover:shadow-md border border-black/[0.04]`}
          >
